...
function AutoCards(inHook, inText, inStop) {
    ...
    */
    ...
    function hoistConst() { return (class Const {
        ...
        constructor(...args) {
            if (args.length !== 0) {
                Const._throwError([[(args.length === 1), "Const cannot be instantiated with a parameter"], ["Const cannot be instantiated with parameters"]]);
            } else {
                O.f(this);
                return this;
            }
        ...
        declare(...args) {
            if (args.length !== 0) {
                Const._throwError([[(args.length === 1), "Instances of Const cannot be declared with a parameter"], ["Instances of Const cannot be declared with parameters"]]);
            } else if (this._constant === undefined) {
                this._constant = null;
                return this;
            } else if (this._constant === null) {
                Const._throwError("Instances of Const cannot be redeclared");
            } else {
                Const._throwError("Instances of Const cannot be redeclared after initialization");
            }
        }
        initialize(...args) {
            if (args.length !== 1) {
                Const._throwError([[(args.length === 0), "Instances of Const cannot be initialized without a parameter"], ["Instances of Const cannot be initialized with multiple parameters"]]);
            } else if (this._constant === null) {
                this._constant = [args[0]];
                return this;
            } else if (this._constant === undefined) {
                Const._throwError("Instances of Const cannot be initialized before declaration");
            } else {
                Const._throwError("Instances of Const cannot be reinitialized");
            }
        }
        read(...args) {
            if (args.length !== 0) {
                Const._throwError([[(args.length === 1), "Instances of Const cannot be read with a parameter"], ["Instances of Const cannot read with any parameters"]]);
            } else if (Array.isArray(this._constant)) {
                return this._constant[0];
            } else if (this._constant === null) {
                Const._throwError("Despite prior declaration, instances of Const cannot be read before initialization");
            } else {
                Const._throwError("Instances of Const cannot be read before initialization");
            }
        }
        // An error condition is paired with an error message [condition, message], call _throwError with an array of pairs to throw the message corresponding with the first true condition [[cndtn1, msg1], [cndtn2, msg2], [cndtn3, msg3], ...] The first conditionless array element always evaluates to true ('else')
        static _throwError(...args) {
            ...
            const error = O.f(new Error((function() {
                ...
                if (Array.isArray(conditionalMessagesTable)) {
                    const chosenPair = conditionalMessagesTable.find(function(...args) {
                        ...
                        if (Array.isArray(pair)) {
                            if ((pair.length === 1) && (typeof pair[0] === "string")) {
                                ...
                                Const._throwError("Const._throwError encountered an invalid array element of conditionalMessagesTable");
                            }
                        } else {
                            Const._throwError("Const._throwError encountered a non-array element within conditionalMessagesTable");
                        }
                    ...
                    if (Array.isArray(chosenPair)) {
                        ...
                        codomain.initialize("Const._throwError was not called with any true conditions");
                    }
                } else if (typeof conditionalMessagesTable === "string") {
                    codomain.initialize(conditionalMessagesTable);
                } else {
                    codomain.initialize("Const._throwError could not parse the given argument");
                }
                return codomain.read();
            })()));
            if (error.stack) {
                codomain.initialize(error.stack
                    ...
                    .replace(/Error:|at\s*(?:_throwError|Const.(?:declare|initialize|read)|new\s*Const)\s*\(\d+:\d+\)/gi, "")
    ...
    function hoistWords() { return (class Words { static _cache = {}; static {
        ...
        for (const wordList in wordListInitializers) {
            ...
            Object.defineProperty(Words, wordList, {
                ...
                get() {
                    ...
                    if (!(wordList in Words._cache)) {
                        Words._cache[wordList] = O.f(wordListInitializers[wordList]());
                    }
                    return Words._cache[wordList];
                }
            });
        }
    } }); }
    function hoistStringsHashed() { return (class StringsHashed {
        ...
        static _defaultSize = 65536;
        _size;
        _store;
        constructor(size = StringsHashed._defaultSize) {
            this._size = size;
            this._store = new Set();
            return this;
        }
        static deserialize(serialized, size = StringsHashed._defaultSize) {
            const stringsHashed = new StringsHashed(size);
            stringsHashed._store = new Set(serialized.split(","));
            return stringsHashed;
        }
        serialize() {
            return Array.from(this._store).join(",");
        }
        has(str) {
            return this._store.has(this._hash(str));
        }
        add(str) {
            this._store.add(this._hash(str));
            return this;
        }
        remove(str) {
            this._store.delete(this._hash(str));
            return this;
        }
        size() {
        ...
        latest(keepLatestCardinality) {
            if (this._store.size <= keepLatestCardinality) {
                return this;
            }
            const excess = this._store.size - keepLatestCardinality;
            const iterator = this._store.values();
            for (let i = 0; i < excess; i++) {
                // The oldest hashcodes are removed first (insertion order matters!)
                this._store.delete(iterator.next().value);
            }
            return this;
        }
        _hash(str) {
            ...
            for (let i = 0; i < str.length; i++) {
                hash = ((31 * hash) + str.charCodeAt(i)) % this._size;
            }
